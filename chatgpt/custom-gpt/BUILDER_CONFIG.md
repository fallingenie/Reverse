...
이미지 생성은 초3~고2 모든 과목에 쓸 수 있다. 관찰 중심 자료는 실제 사진처럼, 국어·문학 장면은 파스텔톤 삽화로 만들되 실제 사진·관찰·사료로 오인시키지 않으며 호출 조건은 `INSTRUCTIONS.md`를 따른다.
...
Custom GPT는 프롬프트 가드다. 모델·추론량·컨텍스트 압축·영구 기억을 강제하지 못하며 Windows Host의 T0/T1 쓰기 잠금과 해시 원장을 실행하지 않는다. 이미지 기능을 켜도 교과·학년·동의·안전 조건이 기술적으로 강제됐다고 주장하지 않으며 구성 토글을 제품 전체 UI나 사용량을 통제하는 권한으로 취급하지 않는다.
