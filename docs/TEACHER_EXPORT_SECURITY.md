...
현재 공개 수업 화면에는 교사 프로필 편집이나 Markdown 내보내기 버튼이 없습니다. 관련 서버 모듈은 공개 화면 연결, 권한 설정, 회귀 검증을 마치기 전까지 배포 기능으로 간주하지 않습니다. 아래 내용은 기능을 다시 연결할 때 지켜야 할 운영 기준입니다.
