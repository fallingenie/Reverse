...
`.editorconfig`가 기본값과 예외를 선언한다. `pnpm run format`은 사람이 편집하는 파일에 BOM을 정확히 하나 유지하고 기계 파일에서는 제거한다. 저장소 검증은 사람이 편집하는 Markdown·YAML·일반 텍스트와 PowerShell의 UTF-8-SIG, JSON·소스·원장·도구 제어 파일의 무BOM을 전수 검사한다.
